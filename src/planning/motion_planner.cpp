...
bool MotionPlanner::isPathSafe(const robot_path_t& path) const
{
    ...
    for (auto pose: path.path) {
        ...
        Point<int> cell = global_position_to_grid_cell(coord, distances_);
        if (!distances_.isCellInGrid(cell.x,cell.y)) {
            return false;
        } else if (distances_(cell.x,cell.y)<searchParams_.minDistanceToObstacle) {
            return false;
        }
