                                   ...
                                   const MotionPlanner& planner)
{
    ...
    */
    ...
    Point<float> close_endpoints;
    Point<float> close_midpoint;
    frontier_t close_frontier;

    float radius = 0.2;
    float step = M_PI/20;

    for (int i=0; i<frontiers.size(); ++i) {
        endpoints = find_endpoints(frontiers[i].cells);
        Point<float> midpoint(0.5*(endpoints[0].x + endpoints[1].x), 0.5*(endpoints[0].y + endpoints[1].y));
        float dist = sqrt((midpoint.x - robotPose.x)*(midpoint.x - robotPose.x) + (midpoint.y - robotPose.y)*(midpoint.y - robotPose.y));
        if (dist < min_dist) {
            min_dist = dist;
            close_midpoint = midpoint;
            close_frontier = frontiers[i];
        }
    }

    Point<float> current_midpoint = close_midpoint;
    std::vector<Point<float>> current_endpoints = endpoints;
    int j = 0;
    while (!is_midpoint_valid(planner, close_midpoint, step, radius) && j<2) {
        float midx = (current_endpoints[0].x + close_midpoint.x)*0.5;
        float midy = (current_endpoints[0].y + close_midpoint.y)*0.5;
        current_midpoint = Point<float>(midx, midy);

        if (is_midpoint_valid(planner, close_midpoint, step, radius)) {
            close_midpoint = current_midpoint;
        }
        current_endpoints[1] = Point<float>(midx, midy); 
        j++;
    }

    j = 0;
    while (!is_midpoint_valid(planner, close_midpoint, step, radius) && j<2) {
        float midx = (current_endpoints[1].x + current_midpoint.x)*0.5;
        float midy = (current_endpoints[1].y + current_midpoint.y)*0.5;
        current_midpoint = Point<float>(midx, midy);

        if (is_midpoint_valid(planner, close_midpoint, step, radius)) {
            close_midpoint = current_midpoint;
        }
        current_endpoints[0] = Point<float>(midx, midy); 
        j++;
    }

    pose_xyt_t goalPose = robotPose;
    goalPose.x = close_midpoint.x;
    goalPose.y = close_midpoint.y;
    goalPose.theta = compute_theta(robotPose, goalPose);
...
std::vector<Point<float>> find_endpoints(const std::vector<Point<float>>& frontier) {

    float min_x = frontier[0].x;
    float max_x = frontier[0].y;
    // int min_i = 0;
    // int max_i = 0;
    std::vector<Point<float>> endpoints;

    for(int i=0; i<frontier.size(); ++i) {
        if (frontier[i].x < min_x) {
            min_x = frontier[i].x;
            // min_i = i;
        }
        if (frontier[i].x > max_x) {
            max_x = frontier[i].x;
            // max_i = i;
        }
    }

    float min_y = frontier[min_x].y;
    float max_y = frontier[max_x].y;
    Point<float> minpoint(min_x, min_y);
    endpoints.push_back(minpoint);
    Point<float> maxpoint(max_x, max_y);
    endpoints.push_back(maxpoint);
    return endpoints;
}

float compute_theta(pose_xyt_t start, pose_xyt_t goal) {
...
bool is_midpoint_valid(const MotionPlanner& planner, Point<float>& current_midpoint, float step, float radius) {

    bool isValid = false;
    float theta = 0.0;
    while (theta < 2*M_PI) {

        // Point<float> current_point(current_midpoint.x + radius * cos(theta), current_midpoint.y + radius * sin(theta);
        pose_xyt_t current_pose;
        current_pose.x = current_midpoint.x + radius * cos(theta);
        current_pose.y = current_midpoint.y + radius * sin(theta);
        current_pose.theta = theta;
        if (planner.isValidGoal(current_pose)) {
            isValid = true;
            current_midpoint = Point<float>(current_pose.x, current_pose.y);
            return isValid;
        }
        theta += step;
    }
